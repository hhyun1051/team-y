...
사용자 입력을 분석하여 다음 다섯 가지 시나리오 중 하나로 분류하세요:
...
**시나리오 4 - business_registration (사업자등록증 등록):**
- 사업자등록증 관련 질의 및 등록 요청
- 거래처 등록, 사업자 정보 입력 관련
- 예시: "사업자 등록해줘", "사업자등록증 처리"
- 예시: "거래처 등록", "새 업체 추가"
- 예시: "사업자번호 등록", "거래처 정보 입력"
...
**분류 기준:**
1. 사업자등록증, 거래처 등록, 업체 추가 관련 키워드 → business_registration
2. 이름이나 수령인 정보가 있으면 → delivery
3. 거래처 정보와 단가가 모두 있으면 → product_order
4. 알루미늄/금속 제품 규격만 있고 단가 계산을 원하면 → aluminum_calculation
5. 도움말 요청, 기능 설명, 인사, 기타 → help
6. 불명확한 경우 문맥과 키워드로 판단
